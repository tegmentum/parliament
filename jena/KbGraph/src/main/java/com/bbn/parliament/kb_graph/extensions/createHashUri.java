...
package com.bbn.parliament.kb_graph.extensions;
...
/// takes a list of arbitrary inputs and creates from them a new URI. It does
/// this by converting the items to strings, combining those into a single
/// string, and applying a cryptographic hash to the result.
///
/// To use this function in a SPARQL query, first declare a prefix like so (the
/// final period is significant!):
///
/// {@snippet :
/// PREFIX ext: <java:com.bbn.parliament.kb_graph.extensions.>
/// }
///
...
/// }
///
/// This will cause the variable {@code ?newUri} to be bound to a hash URI
/// created from {@code ?x}, {@code ?y}, and {@code ?z}. The resulting URI will
...
///
/// <ul>
///    <li>A base URI to which the hash will be appended, and</li>
///    <li>One or more items to hash.</li>
/// </ul>
///
...
/// that unbound variables will not result in a nonsensical URI. For instance, if
/// all of the items in the list were unbound variables, then the resulting URI
/// would not likely be a unique identifier of anything.
///
/// If you are using this method to generate a reification URI fragment, then you
/// need to pass in three strings in this order: subject, predicate, and object.
///
/// Note that because this class's name will appear in SPARQL queries as a
/// function, it begins with a lower case letter in opposition to the usual
...
public class createHashUri implements Function {
	...
	/// words, it takes a variable length list of strings and creates from them a
	/// legal CSV file row. The created row is minimal, in that the elements are
	/// quoted only if they need to be.
	///
	/// @param strings A list of strings from which to create the row.
	/// @return A CSV file row corresponding to the given string array. This does not
	///         include the row's terminal end-of-line character.
	...
	/// Computes the cryptographic hash of a string according to the algorithm given
	/// by ALGORITHM, The result is formatted as a hex string.
	///
	/// @param strToBeHashed The string to be hashed
	/// @return The hex-encoded hash of the input string
